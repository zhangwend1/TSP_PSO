...
double Swarm::solve() {
    ...
    while (moves_since_best_changed <= 4) {
        ...
        iteration_count++;
        //std::cout << "Iteration number:" << iteration_count << "  Best value so far: " << this->best_value << std::endl;
    }
    ...
    //{
    //    std::cout << "(" << this->best_position.nodes[i].x << "," << this->best_position.nodes[i].y << ")" << " ";
    //}
    //std::cout << " " << std::endl;
...
void Swarm::read_graph_definition(std::string filename) {
    ...
    if (graph_file.is_open()) {
        ...
        while (graph_file.good()) {
            ...
            if (n.index > 0) {
                n.index--; // index from 0, file indexes from 1
